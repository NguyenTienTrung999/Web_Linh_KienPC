...
<div class="flex flex-col gap-4 sm:flex-row sm:items-center justify-between mb-8">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-slate-900 dark:text-white text-xl font-bold tracking-tight">Sửa thương hiệu: {{ $brand->name }}</h2>
    </div>
...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
        @csrf
        @method('PUT')
        <div class="p-6 space-y-6">
            <div class="space-y-2">
                <label class="text-sm font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Tên thương hiệu *</label>
                <input name="name" value="{{ old('name', $brand->name) }}" required class="w-full bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 focus:ring-primary focus:border-primary text-slate-900 dark:text-white font-medium" type="text"/>
            </div>

            <div class="space-y-2">
                <label class="text-sm font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Logo thương hiệu</label>
                <div class="flex items-center gap-6">
                    <div id="logo-preview-container" class="w-24 h-24 rounded-xl border border-primary/30 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center overflow-hidden">
                        @if($brand->logo)
